...
class OrderController extends Controller
{
    ...
    public function create()
    {
        ...
        $orders = Order::all();
        return view('admin.order.order', compact('products', 'orders', 'customers', 'shippers'));
    }
    ...
    public function store(Request $request)
    {
        ...
        DB::transaction(function () use ($request) {
            ...
            $type = "Order";
            $transactions->trx_code = $trx_code;
        ...
        return redirect('/admin/orders')->with('status', 'Order Successful');
    }
    public function generateTrxCode()
    {
        $str_code = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ';
        $trx_length = 10;
        $trx = rand(pow(10, $trx_length), pow(10, $trx_length) - 1);
    }
    ...
    public function show(string $id)
    {
        ...
        return view('admin.order.detail', ['orders' => $orders, 'order_details' => $order_details]);
    }
    ...
    public function destroy(string $id)
    {
        ...
        $payment = DB::table('orders')
            ->leftJoin('payments', 'payments.id', 'orders.payment_id')
            ->where('orders.id', $id)->delete();
        // $payment1 = (int)($payment);
        // DB::table('payments')->where('payments.id', $payment)->delete();
        $trx_code = DB::table('orders')
            ->leftJoin('transactions', 'transactions.trx_code', 'orders.trx_code')
            ->where('orders.id', $id)->delete();
        // $trx_code1 = (int)($trx_code);
        // DB::table('transactions')->where('trx_code', $trx_code)->delete();
        // dd($orders, $payment, $trx_code);
